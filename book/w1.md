...
$$\begin{equation}c_n=\frac{f^{(n)}(a)}{n!}\end{equation}$$ <br>
...
``` {image} images/error_comparison.png
:alt: error comparison
:height: 400px
:align: center 
```

The script below provides the code that was used to produce the figure and estimate the slope.
...
$$\begin{equation}\sin(x)+x, \quad x\in[-\pi,\pi],\end{equation}$$ <br> using either the forward or central difference formula. You will need to extend the function estimate at *two* exterior points to obtain a result for the derivative at each point in the interval.
```` 
